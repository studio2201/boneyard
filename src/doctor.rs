//! doctor.rs — System & Environment Diagnostics for boneyard.
...
fn check_binary() -> DoctorCheck {
    let exe = env::current_exe().unwrap_or_else(|_| PathBuf::from("boneyard"));
    if !exe.exists() {
        return DoctorCheck {
            name: "binary_integrity".into(),
            status: "error".into(),
            detail: format!("Binary not found at {}", exe.display()),
            remediation: Some("Reinstall boneyard via install.sh".into()),
        };
    }
